...
void Right_SpeedCount(void) //右编码器中断服务
{
...
{ 
    s32  iError,iIncpid;                          //???? 
		Left_SpeedCount();
		pid_left->PrevError = pid_left->LastError;
    pid_left->LastError = iError;
    iError  = pid_left->SetSpeed - LeftGetSpeed;         //Setspeed设定速度，GetSpeed编码器读取速度  
    iIncpid = (s32)(pid_left->Proportion * (iError - pid_left->LastError)           //E[k]? 
										+ pid_left->Integral * pid_left->LastError
										+ pid_left->Derivative *(iError - 2*pid_left->LastError + pid_left->PrevError));    //E[k-1]?  
    return(iIncpid);                             //????? 
}
...
{ 
    s32  iError,iIncpid;                          //???? 
		Right_SpeedCount();
		pid_right->PrevError = pid_right->LastError;
    pid_right->LastError = iError;
    iError  = pid_right->SetSpeed - RightGetSpeed;         //Setspeed设定速度，GetSpeed编码器读取速度  
    iIncpid = (s32)(pid_right->Proportion * (iError - pid_right->LastError)           //E[k]? 
										+ pid_right->Integral * pid_right->LastError
										+ pid_right->Derivative *(iError - 2*pid_right->LastError + pid_right->PrevError));  
    return(iIncpid);                             //????? 
}
