...
#define angle_times   8
#define angle_numbers 4

#define q30 1073741824.0f
float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
float Pitch = 0,Roll = 0,Yaw = 0; 

float pitch[5] = {0,0,0,0,0};
float roll[5] = {0,0,0,0,0};
float yaw[5] = {0,0,0,0,0};
...
u8 AnBT_DMP_MPU6050_Init(void)	
{
	...
	//AnBT_Uart1_Send_String("P-1,Finish DMP device init.",27); 		//完成初始化
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL))
//		AnBT_Uart1_Send_String("P-2,Finish DMP set sensor.",26);		//完成传感去设定
	//else
//		AnBT_Uart1_Send_String("P-2,DMP set sensor error.",25);
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) 
		//AnBT_Uart1_Send_String("P-3,Finish DMP configure fifo.",30);		//完成fifo设置
	//else AnBT_Uart1_Send_String("P-3,DMP configure fifo error.",29);
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))
		//AnBT_Uart1_Send_String("P-4,Finish DMP set sample rate.",31);			//完成DMP采样率设置
	//else AnBT_Uart1_Send_String("P-4,DMP set sample_rate error.",30);
	//
	if(!dmp_load_motion_driver_firmware())	
		//AnBT_Uart1_Send_String("P-5,Finish DMP load motion driver firmware.",43);				//开启DMP功能
	//else AnBT_Uart1_Send_String("P-5,DMP load motion driver firmware error.",42);
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation)))
//		AnBT_Uart1_Send_String("P-6,Finish DMP set orientation.",31);		//初始方向设定偏差
	//else AnBT_Uart1_Send_String("P-6,DMP set orientation error.",30);
	//
	if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
	DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
	DMP_FEATURE_GYRO_CAL))		//AnBT_Uart1_Send_String("P-7,Finish DMP enable feature.",30);		//DMP使能
	//else AnBT_Uart1_Send_String("P-7,DMP enable feature error.",29);
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) 
		//AnBT_Uart1_Send_String("P-8,Finish DMP set fifo rate.",29);		//DMP  fifo速度
	//else AnBT_Uart1_Send_String("P-8,DMP set fifo rate error.",28);
	//
	run_self_test();
	if(!mpu_set_dmp_state(1)); 
		//AnBT_Uart1_Send_String("P-9,Finish DMP set dmp state.",29);				//DMP成功开启
	//else AnBT_Uart1_Send_String("P-9,DMP set dmp state error.",28);					//Dmp启动失败
	//
	return 0;
}
...
void MPU6050DMP_Init(void)
{
 ...
 dmp_status=AnBT_DMP_MPU6050_Init();
}
...
void Angle_Filter(void)
{
	...
	Pitch = Pitch*angle_times;
	Roll = Roll*angle_times;
	Yaw = Yaw*angle_times;
	
	for(i = 0;i < (angle_numbers-1);i++)
	{
		pitch[i] = pitch[i+1]; 
		roll[i] = roll[i+1];
		yaw[i] = yaw[i+1];
	...
	if(((pitch[angle_numbers-1]-Pitch)<angle_times)||(((Pitch-pitch[angle_numbers-1])<angle_times)&&(Pitch < 180)&&(Pitch > -180)))
		pitch[angle_numbers-1] = Pitch; 
	if(((roll[angle_numbers-1]-Roll)<angle_times)||(((Roll-roll[angle_numbers-1])<angle_times)&&(Pitch < 90)&&(Pitch > -90))) 
		roll[angle_numbers-1] = Roll;
	if(((yaw[angle_numbers-1]-Yaw)<angle_times)||(((Yaw-yaw[angle_numbers-1])<angle_times)&&(Pitch < 180)&&(Pitch > -180))) 
		yaw[angle_numbers-1] = Yaw;
	
	for(i = 0;i < angle_numbers;i++)
	{
		Pitch += pitch[i];
		Roll += roll[i];
		Yaw += yaw[i];
	...
	Pitch = Pitch/angle_numbers;
	Roll = Roll/angle_numbers;
	Yaw = Yaw/angle_numbers;
